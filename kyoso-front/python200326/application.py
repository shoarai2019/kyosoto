...
from lib.silhouette_manager import SilhouetteManager
# import for debug
import datetime
...
# Prepare silhouette manager
silhouette_managers = {}
for background_img_path in config.get('silhouette', 'backgrounds').split(','):
    ip = os.path.splitext(os.path.basename(background_img_path))[0]
    silhouette_managers[ip] = SilhouetteManager(background_img_path, config.get('silhouette', 'person_img_path'))
...
@app.route('/person_detect_img', methods=["GET", "POST"])
def person_detect_img():
    result_object = {}
    if request.method == 'POST':
        files = request.json
        if len(files) > 0:
            frames = [cv2.imdecode(np.fromstring(files[filename].read(), np.uint8), cv2.IMREAD_UNCHANGED)
            ...
            for filename, frame_boxes in zip(files, boxes):
                silhouette_manager = silhouette_managers[filename]
                height, width = silhouette_manager.background_img.shape[:2]
                frame_boxes[:, ::2] *= width
                frame_boxes[:, 1::2] *= height

                frame_silhouette = silhouette_manager.draw_person(frame_boxes)
                _, frame_silhouette = cv2.imencode('.jpg', frame_silhouette)
                frame_silhouette = frame_silhouette.tostring()
                frame_silhouette = base64.b64encode(frame_silhouette).decode('utf-8')

                result_object[filename] = {
                    'img': frame_silhouette,
                    'count': frame_boxes.shape[0]
                }
