...
function strOfPeriod(nowDate, period) {
    let nowYear = nowDate.getFullYear();
    let nowMonth = zeroPadding(nowDate.getMonth() + 1, 2);
    let nowDay = zeroPadding(nowDate.getDate(), 2);

    if(period == 'week') {
        nowDate.setDate(nowDate.getDate() - 6);
        let preYear = nowDate.getFullYear();
        let preMonth = zeroPadding(nowDate.getMonth() + 1, 2);
        let preDay = zeroPadding(nowDate.getDate(), 2);

        return preYear + '.' + preMonth + '.' + preDay + ' - ' + nowYear + '.' + nowMonth + '.' + nowDay
    } else if(period == 'month') {
        nowDate.setMonth(nowDate.getMonth() - 1);
        let preYear = nowDate.getFullYear();
        let preMonth = zeroPadding(nowDate.getMonth() + 1, 2);
        let preDay = zeroPadding(nowDate.getDate() - 1, 2);
...
function displayTop(data) {
    ...
    const floor1 = document.getElementById('gradient1');
    let sum = 0;
    for (let key in Object.keys(data.counts)) {
        sum += data.counts[key];
    }

    let width = (sum * 100) / 45;
    if(width > 100) {width = 100}
    gradient1.style.width = width + '%'
    if(width >= 80) {
        floor1.style.backgroundImage = 'url(../img/top/1F_gauge/1F_5.png)'
        console.log('floor1の人数変更5');
    } else if(width >= 60) {
        floor1.style.backgroundImage = 'url(../img/top/1F_gauge/1F_4.png)'
        console.log('floor1の人数変更4');
    } else if(width >= 40) {
        floor1.style.backgroundImage = 'url(../img/top/1F_gauge/1F_3.png)'
        console.log('floor1の人数変更3');
    } else if(width >= 20) {
        floor1.style.backgroundImage = 'url(../img/top/1F_gauge/1F_2.png)'
        console.log('floor1の人数変更2');
    } else if(width < 20 ) {
        floor1.style.backgroundImage = 'url(../img/top/1F_gauge/1F_1.png)'
        console.log('floor1の人数変更1');
    }
}
