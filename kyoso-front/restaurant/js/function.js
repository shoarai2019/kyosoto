...
function drawDefaultImage(id) {
    // canvas
    var canvas = document.getElementById(id);
    // var canvas2 = document.getElementById('image2');
    // var canvas3 = document.getElementById('image3');
    // var canvas4 = document.getElementById('image4');
    // var canvas5 = document.getElementById('image5');
    // var canvas6 = document.getElementById('imageCafe');
    ...
    if (canvas !== null) {
        ...
        // load image
        var img = new Image();
        img.src = "/restaurant/img/pages/" + id + ".png";
...
function wsopen() {
    if (webSocket === null) {
        // 初期化
        webSocket = new WebSocket(restUri);
        // イベントハンドラ
        ...
        webSocket.onmessage = function(ev) {
            if (ev && ev.data) {
                ...
                console.log('更新:' + JSON.stringify(data));
        ...
        switch ( xhr.readyState ) {
            ...
            case 4: // データ受信完了.
                if( xhr.status == 200 || xhr.status == 304 ) {
                    ...
                    drawImage('imageCafe' , data.images);
                } else {
...
function displayData(data) {
    ...
    for (let i = 1; i < counterTableRows.length; i++) {
        let areaId = counterTableRows[i].id;
        let count = data.counts[areaId];
        updateLocalStrage(areaId, count);
        ...
        if(populationList === 'real') {
            ...
            updateChart(areaId, populationList);
        }
